...
```
    ...
    ## output_profile_file: ./results/ff026585/profiles.parquet
...
```
    ...
    ## Joining with `by = join_by(Metadata_control_type, Metadata_reference_or_other, Metadata_broad_sample)`
...
```
    ...
    ## Warning in inner_join(., profiles_experiment %>% distinct(across(all_of(c(split_single, : Each row in `x` is expected to match at most 1 row in `y`.
    ...
    ## Warning in left_join(., split_mapper_compact, by = element_column): Each row in `x` is expected to match at most 1 row in `y`.
    ## ℹ Row 65 of `x` matches multiple rows.
    ## ℹ If multiple matches are expected, set `multiple = "all"` to silence this warning.
